...
class InternationalModel extends HyAllModel {
    ...
    protected function getOptions_stu_id(){
        $arr = M('user')->alias('u')->join('edu_student s on u.id=s.user_id')->where(array(
            's.status'=>array('lt',9),'u.status'=>array('lt',9),'country' => array('neq','中国')
        ))->getField('id,name');
        return $arr;
    }
