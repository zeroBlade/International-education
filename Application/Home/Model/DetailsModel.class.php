...
//        dump($category_id);
    ...
    public function searchFun($input){
        ...
        $count=$this->alias('d')
        ->join($this->dbPrex.'category AS c ON d.category_id = c.id')
        ->field('title,content,name AS category_name,d.id AS d_id,c.id AS c_id,d.create_time,d.language_id')->order('create_time desc')->where($map)->where(array('c.status'=>1,'d.status'=>1,'d.is_publish'=>array('eq',1)))->count();
        //分页的元素
        $page = $this->articlePage($count,C('SEARCH_ARTICLE_NUM'));
        //所有取得的数据
        $Data = $this->alias('d')
            ->join($this->dbPrex.'category AS c ON d.category_id = c.id')
            ->field('title,content,name AS category_name,d.id AS d_id,c.id AS c_id,d.create_time,d.language_id')->order('create_time desc')->where($map)->where(array('c.status'=>1,'d.status'=>1,'d.is_publish'=>array('eq',1))) ->page($_GET['p'],C('SEARCH_ARTICLE_NUM'))->select();
